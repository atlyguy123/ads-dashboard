#!/usr/bin/env python3
"""
TEST VERSION: Module 1: Download/Update Data
Downloads only 2 days of data for testing purposes.
"""
import os
import sys
import boto3
...
USE_POSTRES = DATABASE_URL is not None and HAS_POSTRES

# TEST CONFIGURATION: Only check last 5 days instead of 90
TEST_DAYS_TO_CHECK = 5
...
def main():
    try:
        print("🧪 === TEST VERSION: Module 1: Download/Update Data ===")
        print(f"🧪 Testing with only {TEST_DAYS_TO_CHECK} days instead of 90")
        logger.info("🧪 === TEST MODULE 1 STARTED: Download/Update Data ===")
        ...
        if latest_date:
            ...
            logger.info("ℹ️  No existing data found - will download from scratch")
        ...
        print(f"📊 Missing data for {len(missing_dates)} days (out of {TEST_DAYS_TO_CHECK} checked)")
        ...
        if missing_dates:
            ...
            if not success:
                print("⚠️  Some downloads failed, but continuing...")
            ...
            if user_success:
                ...
                logger.warning("⚠️  User data download failed, but continuing...")
        ...
        logger.info("🧪 === TEST MODULE 1 COMPLETED SUCCESSFULLY ===")
        return 0
        
    except Exception as e:
        print(f"❌ Test Module 1 failed: {e}", file=sys.stderr)
        logger.error(f"🧪 === TEST MODULE 1 FAILED ===")
...
def download_and_store_event_file(conn, db_type, s3_client, bucket_name, object_key, target_date, file_sequence):
    """Downloads an event .json.gz file from S3, decompresses it, filters events by name, and stores in database."""
    cursor = conn.cursor()
...
def download_and_store_user_file(conn, db_type, s3_client, bucket_name, object_key):
    """Downloads a user profile .json.gz file from S3, decompresses it, and stores all users in database."""
    cursor = conn.cursor()
